...
def view_jobs(request):
    ...
    completed_job_ids = JobApplications.objects.filter(employer_id=employer, status='Completed').values_list('job_id', flat=True)

    # Fetch jobs using the obtained job_ids
    completed_jobs = Job.objects.filter(id__in=completed_job_ids).order_by('-id')
...
def job_details(request,id):
    ...
    # Retrieve previous reviews and ratings for the worker
    worker_reviews = JobApplications.objects.filter(
    job_id__worker_id=job.worker_id,
